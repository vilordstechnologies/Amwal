...
class Amwal_Social_Links_Widget extends WP_Widget {
	protected $default;
	protected $socials;
	...
	function __construct() {
		$this->socials = array(
			...
			'google-plus'  => esc_html__( 'Google Plus', 'amwal' ),
			'youtube-play' => esc_html__( 'Youtube', 'amwal' ),
		...
		parent::__construct(
			'social-links-widget',
			esc_html__( 'Amwal - Social Links', 'amwal' ),
			array(
				'classname'   => 'social-links-widget social-links',
				'description' => esc_html__( 'Display links to social media networks.', 'amwal' ),
			),
	...
	function widget( $args, $instance ) {
		...
		foreach ( $this->socials as $social => $label ) {
			if ( ! empty( $instance[$social . '_url'] ) ) {
				printf(
					'<a href="%s" class="share-%s  social" rel="nofollow" title="%s"  target="_blank"><i class="fa fa-%s"></i></a>',
	...
	function form( $instance ) {
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'amwal' ); ?></label>
